...
void L6470::setMaxSpeed(int speed) {
  ...
  SetParam(L6470_MAX_SPEED, MaxSpdCalc(speed));
}
...
void L6470::setMinSpeed(int speed) {
  ...
  //  steps/s into an appropriate value for this function. Note that for any move or
  //  goto type function where no speed is specified, this value will be used.
  SetParam(L6470_MIN_SPEED, MinSpdCalc(speed));
}
...
void L6470::setAcc(float acceleration) {
  ...
  SetParam(L6470_ACC, AccCalc(acceleration));
}


void L6470::setDec(float deceleration) {
  SetParam(L6470_DEC, DecCalc(deceleration));
}


long L6470::getPos() {
  return convert(GetParam(L6470_ABS_POS));
}
...
void L6470::setStallCurrent(float ma_current) {
  ...
  SetParam(L6470_STALL_TH, STHValue);
}
...
unsigned long L6470::ParamHandler(const byte param, const unsigned long value) {
  ...
  switch (param) {
    ...
    case L6470_STATUS:  // L6470_STATUS is a read-only register
      ret_val = Param(0, 16);
      break;
    default:
      ret_val = Xfer((byte)(value));
